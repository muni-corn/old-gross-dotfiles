...
do
    ...
    do
        if [ $entry_a != $entry_b ]
        then
            TILES[$index]="Dual ${MONITORS[$entry_a]} →  ${MONITORS[$entry_b]}"
...
do
    ...
    do
        ...
        then
            ...
            COMMANDS[$index]="xrandr --output ${MONITORS[$entry_a]} --auto \
                              --output ${MONITORS[$entry_a]} --auto --same-as ${MONITORS[$entry_b]}"
...
# Call menu
SEL=$( gen_entries | rofi -dmenu -p "Monitor layout" -a 0 -no-custom  | awk '{print $1}' )
