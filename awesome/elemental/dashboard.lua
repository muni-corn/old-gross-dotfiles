...
local disk_icon = wibox.widget {
    ...
    markup = helpers.colorize_text("\u{F02CA}", colors.color4),
    widget = wibox.widget.textbox()
}
...
local disk_box = create_boxed_widget(disk, dpi(150), dpi(150), colors.background)
...
local bookmarks = wibox.widget {
    create_bookmark("home", os.getenv("HOME"), colors.color1, colors.color9),
    create_bookmark("downloads", user.dirs.downloads, colors.color2, colors.color10),
    create_bookmark("music", user.dirs.music, colors.color6, colors.color14),
    create_bookmark("pictures", user.dirs.pictures, colors.color4, colors.color12),
    create_bookmark("wallpapers", user.dirs.wallpapers, colors.color5, colors.color13),
    create_bookmark("screenshots", user.dirs.screenshots, colors.color3, colors.color11),
    spacing = dpi(10),
    layout = wibox.layout.fixed.vertical
}

local bookmarks_box = create_boxed_widget(bookmarks, dpi(200), dpi(300), colors.background)
...
local update_fortune = function()
    awful.spawn.easy_async_with_shell(fortune_command, function(out)
        -- Remove trailing whitespaces
        out = out:gsub('^%s*(.-)%s*$', '%1')
        fortune.markup = "<i>"..helpers.colorize_text(out, colors.color4).."</i>"
    end)
end
...
local fortune_box = create_boxed_widget(fortune_widget, dpi(300), dpi(140), colors.background)
...
local petal_top_left = create_url_petal("GH", colors.color4, colors.color15, "https://github.com/elenapan/dotfiles", true, true, false, true)
local petal_top_right = create_url_petal("YT", colors.color1, colors.color15, "https://youtube.com/", true, true, true, false)
local petal_bottom_right = create_url_petal("4C", colors.color2, colors.color15, "https://4chan.org/",false, true, true, true)
local petal_bottom_left = create_url_petal("RD", colors.color3, colors.color15, "https://reddit.com/",true, false, true, true)
